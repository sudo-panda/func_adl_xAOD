        ...
        while True:
            ...
            if dr['done'] or (self._partial_ds_ok and len(dr['files']) > 0):
                if not self._quiet and len(phases) > 1:
                    ...
                    log.info('Where we spent time waiting for column data:')
                    for k in phases.keys():
                        log.info(f'  {k}: {phases[k]*100/total}%')

                if dr['phase'] != 'done':
                    # Yikes! Something bad happened. Try to assemble a error messages and blow up.
                    with StringIO() as s:
                        log.error('The request failed:', file=s)
                        if 'message' in dr:
                            log.error(f'  Message: {dr["message"]}', file=s)
                        if 'log' in dr:
                            log.error('  Log lines:', file=s)
                            for line in dr['log']:
                                log.error(f'    {line}', file=s)
                        raise FuncADLServerException(s.getvalue())

                r = {'files': self.extract_filespec(dr)}
                if not self._quiet:
                    log.info('Files that were returned:')
                    for f in r['files']:
                        log.info(f'  {f}')
            ...
            if not self._quiet:
                if print_count == 0:
                    ...
                    log.info(f'{localtime} : Status: {dr["phase"]}')
