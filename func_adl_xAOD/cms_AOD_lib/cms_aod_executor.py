...
from func_adl_xAOD.common_lib.cpp_functions import find_known_functions
from func_adl_xAOD.cms_AOD_lib.ast_to_cpp_translator import query_ast_visitor

cmsAODExecutionInfo = namedtuple('cmsAODExecutionInfo', 'result_rep output_path main_script all_filenames')
...
class cms_aod_executor:
    ...
    def write_cpp_files(self, ast: ast.AST, output_path: Path) -> cmsAODExecutionInfo:
        r"""
        ...
        template_dir = _find_dir("func_adl_xAOD/template/cms/r5")
        j2_env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(template_dir))
        self._copy_template_file(
            j2_env, info, 'analyzer_cfg.py', output_path)
        self._copy_template_file(j2_env, info, 'Analyzer.cc', output_path)
        self._copy_template_file(j2_env, info, 'BuildFile.xml', output_path)
        self._copy_template_file(j2_env, info, 'runner.sh', output_path)

        (output_path / 'runner.sh').chmod(0o755)

        # Build the return object.
        return cmsAODExecutionInfo(result_rep, output_path, 'runner.sh', ['ATestRun_eljob.py', 'package_CMakeLists.txt', 'query.cxx', 'query.h', 'runner.sh'])
