...
import func_adl_xAOD.cms_AOD_lib.EventCollections
...
class book_cmsaod_ttree(statement.book_ttree):
...
class cmsaod_ttree_fill(statement.ttree_fill):
    'Fill a CMS TTree'
...
class query_ast_visitor(FuncADLNodeVisitor):
    ...
    def get_as_ROOT(self, node: ast.AST) -> rh.cpp_ttree_rep:
        ...
        if isinstance(values, crep.cpp_dict):
            ...
            ast_ttree = function_call('ResultTTree',
                                      [ast_dummy_source,
                                       col_names,
                                       ast.parse('"cmsaod_tree"').body[0].value,  # type: ignore
                                       ast.parse('"cmsaod_output"').body[0].value])  # type: ignore
    ...
    def make_sequence_from_collection(self, rep):
        ...
        l_statement = statement.loop(iterator_value, crep.dereference_var(rep), is_loop_var_ref=True)  # type: ignore
    ...
    def call_ResultTTree(self, node: ast.Call, args: List[ast.AST]):
        ...
        self._gc.add_book_statement(book_cmsaod_ttree(tree_name, var_names))
        ...
        self._gc.add_statement(cmsaod_ttree_fill(tree_name))
