...
from func_adl_xAOD.cms_AOD_lib.cms_aod_executor import cms_aod_executor
...
class LocalFile(EventDataset):
    ...
    async def execute_result_async(self, a: ast.AST) -> Any:
        ...
        with tempfile.TemporaryDirectory() as local_run_dir_p:
            ...
            docker_cmd = f'docker run --rm -v {f_spec.output_path}:/scripts -v {f_spec.output_path}:/results {datafile_mount} cmsopendata/cmssw_5_3_32 /scripts/{f_spec.main_script}'
            ...
            if dump_cpp or proc.returncode != 0:
                level = logging.INFO if proc.returncode == 0 else logging.ERROR
                lg = logging.getLogger(__name__)
                with open(os.path.join(str(local_run_dir), "Analyzer.cc"), 'r') as f:
