...
def test_good_ev_call():
    ...
    doit = function_call('Jets', [ast.parse('"antikt"').body[0].value])
...
def test_good_ev_call_bad_arg_type():
    'Bad collection type'
    doit = function_call('Jets', [ast.parse('55').body[0].value])
...
def test_good_ev_call_arg_number_bad():
    ...
    doit = function_call('Jets', [ast.parse('"antikt"').body[0].value, ast.parse('55').body[0].value])
